    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden md:flex flex-col flex-1 overflow-y-auto">
        <div class="px-2 py-4 space-y-1">
            ...
            @if(auth()->user()->role === 'admin')
                ...
                <a href="{{ route('admin.categories') }}" wire:navigate class="block px-4 py-3 rounded-xl text-base font-medium {{ request()->routeIs('admin.categories*') ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-300 hover:bg-gray-700 hover:text-white transition-all' }}">
                    <div class="flex items-center">
                        <svg class="w-6 h-6 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path></svg>
                        Kategori Soal
                    </div>
                </a>
                ...
                <a href="{{ route('admin.pengawas') }}" wire:navigate class="block px-4 py-3 rounded-xl text-base font-medium {{ request()->routeIs('admin.pengawas*') ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-300 hover:bg-gray-700 hover:text-white transition-all' }}">
                    <div class="flex items-center">
                        <svg class="w-6 h-6 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        Manajemen Pengawas
                    </div>
            ...
            @elseif(auth()->user()->role === 'pengawas')
                <a href="{{ route('pengawas.dashboard') }}" wire:navigate class="block px-4 py-3 rounded-xl text-base font-medium {{ request()->routeIs('pengawas.dashboard*') || request()->routeIs('pengawas.exams.*') ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-300 hover:bg-gray-700 hover:text-white transition-all' }}">
                    <div class="flex items-center">
                        <svg class="w-6 h-6 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                        Monitor Ujian
                    </div>
