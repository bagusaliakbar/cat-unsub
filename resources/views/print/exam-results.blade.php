...
<body class="p-4 sm:p-8 max-w-4xl mx-auto text-black text-[11pt]" onload="window.print()">
    ...
    <div class="text-center mb-8">
        <h3 class="text-lg font-bold uppercase underline">DAFTAR HASIL UJIAN SELEKSI TERTULIS BERBASIS CAT</h3>
    ...
    <table class="w-full table-bordered text-center mb-8">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="align-middle {{ $session->violation_count > 0 ? 'text-red-600 font-bold' : '' }}">
                    {{ $session->violation_count }}x
                </td>
                <td class="align-middle font-bold">{{ round($session->score) }}</td>
                <td class="align-middle font-bold {{ $session->score >= $exam->passing_grade ? 'text-green-600' : 'text-red-600' }}">
